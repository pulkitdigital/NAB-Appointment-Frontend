...
const Book = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-4 sm:py-6 md:py-8 px-3 sm:px-4">
      ...
      <div className="max-w-7xl mx-auto mb-4 sm:mb-6 md:mb-8">
        ...
        >
          <div className="flex items-start sm:items-center gap-3 sm:gap-4">
            <div className="p-2 sm:p-3 bg-gray-900 rounded-lg sm:rounded-xl flex-shrink-0">
      ...
      {/* Main Content - Mobile: Form First, Desktop: Two Columns */}
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-4 sm:gap-5 md:gap-6">

          {/* LEFT COLUMN - Calendar & Time (Mobile: Order 2, Desktop: Left) */}
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.1 }}
          ...
          >
            ...
            {selectedDate && formData.ca_id && slotDurations.length > 0 && (
              ...
              >
                <div className="flex items-center gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                  ...
                  <h3 className="text-base sm:text-lg font-bold text-gray-900">Select Duration</h3>
                </div>
                  ...
                  {slotDurations.map((duration) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                        <svg className={`w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 ${selectedDuration?.duration === duration.duration ? 'text-gray-900' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        {selectedDuration?.duration === duration.duration && (
                          <motion.svg initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-4 h-4 sm:w-5 sm:h-5 text-gray-900 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                          </motion.svg>
                        )}
                      </div>
                      <div className="text-left">
                        <div className="text-sm sm:text-base md:text-lg font-bold text-gray-900">{duration.duration} min</div>
                        <div className="text-xl sm:text-2xl font-bold text-gray-900">₹{duration.price}</div>
                      </div>
                    </motion.button>
                  ))}
                </div>
            ...
            {selectedDate && !formData.ca_id && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-amber-50 border border-amber-200 rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6"
              >
                <div className="flex items-start gap-2 sm:gap-3">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-amber-900 mb-1 text-sm sm:text-base">CA Selection Required</p>
                    <p className="text-xs sm:text-sm text-amber-700">Please select a Chartered Accountant from the form to view available time slots</p>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          {/* RIGHT COLUMN - Form & Summary (Mobile: Order 1 (First), Desktop: Right, Sticky on Desktop) */}
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.2 }}
          ...
          >
            <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 p-4 sm:p-5 md:p-6 lg:sticky lg:top-4 xl:top-8 hover:shadow-md transition-shadow">
              ...
              {isFormComplete() && (
                ...
                >
                  ...
                  <div className="bg-gray-50 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 space-y-2 sm:space-y-3 border border-gray-200">
                    <div className="flex items-center justify-between p-2.5 sm:p-3 bg-white rounded-lg border border-gray-200">
                      <span className="text-xs sm:text-sm text-gray-600 flex items-center gap-1.5 sm:gap-2 font-medium">
                      ...
                      <span className="font-bold text-gray-900 text-xs sm:text-sm md:text-base truncate ml-2">{format(selectedDate, 'EEE, MMM d, yyyy')}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-2.5 sm:p-3 bg-white rounded-lg border border-gray-200">
                      ...
                      <span className="font-bold text-gray-900 text-xs sm:text-sm md:text-base">{format(new Date(`2000-01-01T${selectedSlot}`), 'h:mm a')}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-2.5 sm:p-3 bg-white rounded-lg border border-gray-200">
                      ...
                      <span className="font-bold text-gray-900 text-xs sm:text-sm md:text-base">{selectedDuration.duration} min</span>
                    </div>

                    <div className="pt-2 sm:pt-3 border-t border-gray-300">
                      <div className="flex items-center justify-between p-3 sm:p-4 bg-gray-900 rounded-lg sm:rounded-xl">
                        <span className="text-sm sm:text-base font-semibold text-white">Total Amount</span>
                        <span className="text-2xl sm:text-3xl font-bold text-white">₹{selectedDuration.price}</span>
                      </div>
                    </div>
                  </div>
              ...
              {/* Progress Guide - Only show when form is incomplete */}
              {!isFormComplete() && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-gray-200"
                >
                  <div className="bg-gray-50 border border-gray-200 rounded-lg sm:rounded-xl p-4 sm:p-5">
                    <div className="flex items-start gap-2 sm:gap-3">
                      <svg className="w-5 h-5 sm:w-6 sm:h-6 text-gray-700 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                      </svg>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">Complete these steps to book:</h4>
                        <ul className="space-y-1.5 sm:space-y-2">
                          <li className={`flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm ${formData.name && formData.email && formData.mobile && formData.ca_id && formData.consult_note ? 'text-green-700' : 'text-gray-700'}`}>
                            {formData.name && formData.email && formData.mobile && formData.ca_id && formData.consult_note ? (
                              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                              </svg>
                            ) : (
                              <span className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full bg-gray-200 text-gray-900 text-[10px] sm:text-xs font-bold flex-shrink-0">1</span>
                            )}
                            <span className={`${formData.name && formData.email && formData.mobile && formData.ca_id && formData.consult_note ? 'line-through' : 'font-medium'} truncate`}>
                              Fill in all contact details & select CA
                            </span>
                          </li>
                          <li className={`flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm ${selectedDate ? 'text-green-700' : 'text-gray-700'}`}>
                            {selectedDate ? (
                              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                              </svg>
                            ) : (
                              <span className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full bg-gray-200 text-gray-900 text-[10px] sm:text-xs font-bold flex-shrink-0">2</span>
                            )}
                            <span className={`${selectedDate ? 'line-through' : 'font-medium'} truncate`}>Select a date from the calendar</span>
                          </li>
                          <li className={`flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm ${selectedDuration ? 'text-green-700' : 'text-gray-700'}`}>
                            {selectedDuration ? (
                              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                              </svg>
                            ) : (
                              <span className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full bg-gray-200 text-gray-900 text-[10px] sm:text-xs font-bold flex-shrink-0">3</span>
                            )}
                            <span className={`${selectedDuration ? 'line-through' : 'font-medium'} truncate`}>Choose consultation duration</span>
                          </li>
                          <li className={`flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm ${selectedSlot ? 'text-green-700' : 'text-gray-700'}`}>
                            {selectedSlot ? (
                              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                              </svg>
                            ) : (
                              <span className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full bg-gray-200 text-gray-900 text-[10px] sm:text-xs font-bold flex-shrink-0">4</span>
                            )}
                            <span className={`${selectedSlot ? 'line-through' : 'font-medium'} truncate`}>Pick an available time slot</span>
                          </li>
                        </ul>
                      </div>
                    </div>
