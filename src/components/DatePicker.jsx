    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">Choose your preferred appointment date</p>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-[10px] sm:text-xs text-gray-500 font-medium">Available Days</p>
          <p className="text-xl sm:text-2xl font-bold text-gray-900">{advanceBookingDays}</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-6 sm:py-8">
          ...
          <p className="mt-2 text-sm sm:text-base text-gray-600 font-medium">Loading calendar...</p>
        </div>
      ) : (
        <>
          {/* Calendar Container */}
            ...
            <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2 sm:mb-3">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                <div key={day} className="text-center">
                  <span className="text-[10px] sm:text-xs font-bold text-gray-700 uppercase">{day}</span>
                </div>
              ))}
            </div>

            {/* Calendar Grid */}
            <div className="grid grid-cols-7 gap-1 sm:gap-2 max-h-72 sm:max-h-96 overflow-y-auto custom-scrollbar">
              {calendarDates.map((date, index) => {
                ...
                return (
                  <motion.button
                    ...
                    disabled={isDisabled}
                    className={`
                      relative p-1.5 sm:p-2 md:p-3 rounded-lg text-center transition-all font-semibold
                      ${isSelected 
                        ...
                        : 'bg-white border border-gray-300 text-gray-700 hover:border-gray-400 hover:bg-gray-50 active:bg-gray-100'
                      }
                    `}
                  >
                    ...
                    <div className="text-xs sm:text-sm md:text-base font-bold">{format(date, 'd')}</div>
                    <div className="text-[8px] sm:text-[10px] uppercase mt-0.5">{format(date, 'MMM')}</div>
                  </motion.button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="flex items-center gap-1.5 sm:gap-2 bg-gray-50 border border-gray-200 rounded-lg p-2 sm:p-3">
              ...
              <span className="text-[10px] sm:text-xs font-semibold text-gray-700 truncate">Available</span>
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 bg-gray-50 border border-gray-200 rounded-lg p-2 sm:p-3">
              <div className="w-3 h-3 sm:w-4 sm:h-4 bg-gray-900 rounded flex-shrink-0"></div>
              <span className="text-[10px] sm:text-xs font-semibold text-gray-700 truncate">Selected</span>
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 bg-red-50 border border-red-200 rounded-lg p-2 sm:p-3">
              ...
              <span className="text-[10px] sm:text-xs font-semibold text-gray-700 truncate">Off Day</span>
            </div>
          </div>
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] sm:text-xs font-semibold text-green-700 uppercase">Selected Date</p>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 sm:p-4">
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs sm:text-sm font-semibold text-gray-900">Booking Information</p>
